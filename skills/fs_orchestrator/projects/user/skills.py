...
class SkillsOrchestrator:
    ...
    def list_skills(self, domain: str = None) -> List[str]:
        ...
        if domain:
            return [s for s in self.skills_manifest.get(domain, [])]
        return list(self.skills_manifest.keys())

    def get_skill(self, skill_name: str) -> Dict[str, Any]:
        """Get detailed skill information"""
        for domain, skills in self.skills_manifest.items():
            if skill_name in skills:
                return {"domain": domain, "skill": skill_name}
        return None
